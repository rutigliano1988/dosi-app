...
describe('backfillCandidates', () => {
  ...
  it('genera una fila por día y hora esperada, ventana today-(days-1)…today-1', () => {
    const rows = backfillCandidates('u1', [med({ schedule: { freq: 'daily', times: ['08:00', '20:00'] } })], 3, new Set(), now);
    // días 26, 27 (no el 28 ni el 25) × 2 tomas = 4
    expect(rows).toHaveLength(4);
    expect(rows[0]).toMatchObject({
      user_id: 'u1', med_id: 'm1', date: '2026-08-26', time: '08:00',
      total_min: 480, status: 'upcoming', id: 'm1-2026-08-26-08:00',
    });
    expect(rows.every(r => r.date !== '2026-08-28')).toBe(true);
    expect(rows.every(r => r.date !== '2026-08-25')).toBe(true);
  });
  ...
  it('no repite filas que ya existen', () => {
    ...
    expect(rows.map(r => r.date)).toEqual(['2026-08-26']);
  });
  ...
  it('freq weekdays: solo los días que casan con weekdays generan candidatos', () => {
    // weekdays [1,3] = lunes y miércoles. Ventana days=5 → 24(lun) 25(mar) 26(mié) 27(jue)
    const rows = backfillCandidates('u1', [med({
      schedule: { freq: 'weekdays', times: ['09:00'], weekdays: [1, 3] },
    })], 5, new Set(), now);
    expect(rows.map(r => r.date)).toEqual(['2026-08-24', '2026-08-26']);
    expect(rows.every(r => r.time === '09:00')).toBe(true);
  });

  it('dos medicinas en una llamada: candidatos ordenados por día y luego por hora', () => {
    const a = med({ id: 'a', schedule: { freq: 'daily', times: ['08:00'] } });
    const b = med({ id: 'b', schedule: { freq: 'daily', times: ['12:00'] } });
    const rows = backfillCandidates('u1', [a, b], 3, new Set(), now);
